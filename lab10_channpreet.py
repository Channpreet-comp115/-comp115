...
Your answer is: None, because there is no return statement in function practice_function(nums)
...
Your answer is: Yes. Both the functions have the same functionality. Both returns the 
index of the first matching target, otherwise returns -1.
...
functionality? Why? (5 marks)
Your answer is: No. The function practice_for_index_2(nums, target) returns the index of 
the first matching target, otherwise return -1. 
Whereas, the function practice_for_index_3(nums, target) returns the index of the
last matching target, if not found, it returns -1.
...
Your answer is: It will display all the numbers from the list except target. 
The output of practice_for_continue([5, 5, 2, 9, 6], 2) will be 
5
5
9
6
...
assert exist_in_both("rhythm","help") == 2
