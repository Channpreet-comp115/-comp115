...
def draw_regular_polygon(num_sides, side_length):
    ...
    for _ in range(num_sides):
        pen.forward(side_length) # 10 pixels
        pen.left(exterior_angle) # Angle        
    pen.shape("blank")
    pen.clear()
...
def draw_concentric_circles(num_circles):
    ...
    pen.up()
    for _ in range(num_circles):
        pen.right(90)    # Face South
        pen.forward(radius)   # Move one radius
        pen.right(270)   # Back to start heading
        pen.down()    # Put the pen back down
        pen.circle(radius)    # Draw a circle
        radius+=radius_increase
        pen.up()      # Pen up while we go home
        pen.home()
    pen.shape("blank")
    pen.clear()
