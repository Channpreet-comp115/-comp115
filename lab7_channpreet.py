...
def remove_duplicates_set(s):
    """
    This function removes the duplicates from the string s using set data type
...
def gem_counting(stones, gems):
    """
     This function counts the same characters from a pair of string.

    Parameters:
    - stones (string), gems (string): Two strings from which same characters are count

    Returns:
    - (integer): Count the number of same characters in both strings
    """
    
    count=0
    a = list(set(stones) & set(gems))
    for i in a:     
        count+=1
...
def students_id(ids):
    """
    This function returns unique student ids of students who use the shuttle service.

    Parameters 

    - ids (List) : This list from which duplicate ids are removed

    Returns

    - integer : Return the count of unique ids in the list

    """
    student_list = []
    count=0
    for item in ids:
        if item not in student_list:
...
def students_id_occurences(ids):
    """
   This function returns the occurences of each different student id 

    Parameters 

    - ids (List) : This list of student ids from which count the occurences of each individual student id

    Returns

    - (dictionary): Return the occurences of each individual student id
    """
    count = {}
    for item in ids:
...
assert students_id_occurences(['002', '003', '001', '004', '012']) == {'002': 1, '003': 1, '001': 1, '004': 1, '012': 1}
...
def word_frequency(paragraph):
    """
    This function returns the frequency of words in paragraph

    Parameters 

    - paragraph : The function takes a paragraph 

    Returns

    - (dictionary): Return the frequency of each word in paragraph
    """
